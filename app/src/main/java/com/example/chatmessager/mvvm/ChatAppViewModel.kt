...
import com.example.chatmessager.MyApplication
...
import com.example.chatmessager.notifications.entity.NotificationData
import com.example.chatmessager.notifications.entity.PushNotification
import com.example.chatmessager.notifications.entity.Token
import com.example.chatmessager.notifications.network.RetrofitInstance
...
class ChatAppViewModel:ViewModel() {
    ...
    //sendMessage
    //function using the lauch builder , which it typically used for asynchoronous operations. it seems to be part of an android application, possibly within a viewmodel
    fun sendMessage(sender:String, receiver:String,friendname:String,friendimage:String)=viewModelScope.launch(Dispatchers.IO) {
        ...
        mysharedPrefs.setValue("chatrooomid",uniqueId.toString())
        ...
        firestore.collection("Messages").document(uniqueId.toString()).collection("chats")
            .document(Utils.getTime()).set(hashMap).addOnCompleteListener{
                ...
                firestore.collection("Conversation${Utils.getUidLoggedIn()}").document(receiver).set(setHashMap)
                ...
                firestore.collection("Tokens").document(receiver).addSnapshotListener{
                    value,error->
                    if(value!=null && value.exists()){
                        ...
                        if(message.value!!.isNotEmpty()&& receiver.isNotEmpty()){
                            PushNotification(NotificationData(loggedInUserName,message.value!!),token!!).also {
                                sendNotification(it)
                            }
                        }
                        else{
    ...
    fun updateProfile()=viewModelScope.launch  (Dispatchers.IO){
        ...
        val mysharedPrefs=SharedPrefs(context)
        val friendid=mysharedPrefs.getValue("friedid")
